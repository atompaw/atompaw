...
CONTAINS
  SUBROUTINE Report_Graphatom(key,Grid,Orbit,Pot,SCF)
    ...
    INTEGER :: n,norbit,nps,npp,npd,npf,npg
    ...
    IF (key=='AE') THEN
       ...
       DO i = 1, n  
          IF (r (i) .LE.6.d0) THEN  
             WRITE (ifden, '(1p6e12.4)') r (i), den (i)  
          ENDIF
       ENDDO
       CLOSE (ifden)  
       ...
       DO l = 0, 4  
          ...
          IF (many.GT.0) THEN  
             ...
             DO i = 1, n  
                IF (r (i) .LE.6.d0) THEN  
                   WRITE (ifwfn, '(1p8e10.2)') r (i) , (wav (i, j) , j = &
